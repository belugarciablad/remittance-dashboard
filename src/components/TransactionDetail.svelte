...
{#if isDetailModalOpen}
  <div 
    ...
    aria-modal="true"
  >
    <div class="bg-white p-6 rounded-lg shadow-lg w-96 relative flex flex-col justify-center items-center">
      <!-- Close Button -->
      <button 
      ...
      <div class="text-gray-700 space-y-2 self-start flex flex-col w-full mt-8">
            <div class="transaction-detail-header flex justify-between w-full">
                <h3 id="modal-title" class="text-xl font-semibold mb-4 text-gray-800">#{selectedTransaction.transaction_id}</h3>
                <p class="text-sm font-medium">
                  <span 
                    class="w-20 h-8 text-sm rounded-md flex items-center justify-center {getColorByStatus(selectedTransaction.status)}"
                  >
          ...
          <p><strong>Sender:</strong> {formatPhoneNumber(selectedTransaction.sender_whatsapp)}</p>
          <p><strong>Receiver:</strong> {formatPhoneNumber(selectedTransaction.receiver_whatsapp)}</p>
          <p><strong>Method:</strong> {selectedTransaction.payment_method}</p>
          <p><strong>Amount Sent:</strong> {selectedTransaction.amount_sent}</p>
          <p><strong>Amount Received:</strong> {selectedTransaction.amount_received}</p>
          <p><strong>Currency:</strong> {selectedTransaction.amount}</p>
        ...
        <div class="buttons flex justify-between w-full">
            ...
            <button 
                ...
                on:click={repeatTransaction(selectedTransaction)}
                >
